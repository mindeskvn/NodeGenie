...
exports.NdeMindMapPanel = exports.NdeEditorProvider = void 0;
exports.buildMindMapHtml = buildMindMapHtml;
const vscode = __importStar(require("vscode"));
const ndeParser_1 = require("./ndeParser");
// ──────────────────────────────────────────────────
// Custom Editor Provider
// ──────────────────────────────────────────────────
class NdeEditorProvider {
    static register(context) {
        return vscode.window.registerCustomEditorProvider(NdeEditorProvider.viewType, new NdeEditorProvider(context), {
    ...
    render(webviewPanel, document) {
        try {
            const doc = (0, ndeParser_1.parseNde)(document.getText());
            const scriptUri = webviewPanel.webview.asWebviewUri(vscode.Uri.joinPath(this.context.extensionUri, 'media', 'mindmap.js'));
...
exports.NdeEditorProvider = NdeEditorProvider;
NdeEditorProvider.viewType = 'nodegenie.mindMapEditor';
// ──────────────────────────────────────────────────
// Standalone panel (cho command "Open Mind Map")
// ──────────────────────────────────────────────────
class NdeMindMapPanel {
    ...
    static show(extensionUri) {
        try {
            ...
            const panel = vscode.window.createWebviewPanel('nodegenieMindMapPanel', 'NodeGenie Mind Map', { viewColumn: vscode.ViewColumn.Beside, preserveFocus: true }, {
                enableScripts: true,
    ...
    update(doc) {
        try {
            const scriptUri = this.panel.webview.asWebviewUri(vscode.Uri.joinPath(this.extensionUri, 'media', 'mindmap.js'));
...
exports.NdeMindMapPanel = NdeMindMapPanel;
// ──────────────────────────────────────────────────
...
function buildGraph(doc) {
    ...
    function addNode(id, kind, desc, warn) {
        if (seen.has(id)) {
            return;
        }
        seen.add(id);
    ...
    function walkNodes(list, parentId) {
        for (const n of list) {
...
// ──────────────────────────────────────────────────
function buildMindMapHtml(doc, scriptUri) {
