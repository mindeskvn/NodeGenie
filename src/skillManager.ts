...
// ──────────────────────────────────────────────────
export function ensureSkillExists(context: vscode.ExtensionContext): void {
  try {
...
function createSkillForWorkspace(workspaceRoot: string): void {
  try {
    ...
    vscode.window.showInformationMessage(
      `NodeGenie: Đã tạo skill cho AI agent tại .agents/skills/nodegenie/SKILL.md`,
      'Mở file'
    ).then(btn => {
      if (btn === 'Mở file') {
...
// ──────────────────────────────────────────────────
export function watchWorkspaceFolders(): vscode.Disposable {
